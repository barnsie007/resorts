...
### Step 2 Tasks
1. If using docker ensure the docker service is running. If itâs not, start it:
```
...
```
2. If using docker ensure the docker service is running. If itâs not, start it:
```
service docker start
```
...
3. Deploy your image with its accompanying operator using the following instructions:
   ...
   f. You can view the status of your deployment by running oc get deployments. If you donât see the status after a few minutes, query the pods and then fetch the Liberty pod logs:
    ```
    oc get pods
    oc logs <pod>
    ```
